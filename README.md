...
### 實際 eval 輸出（semantic eval, gpt-4o-mini）

> **L4:** "broadband noise, severity: severe, multiple directions, modulated"
> **L5:** "busy and crowded shopping center with multiple noise sources"
> **L6:** beam=0° width=30°, NR=0.3, compression=1.86
>
> LLM 從純數值無法區分「菜市場」和「百貨公司」— SNR/RT60/能量相同。
> Integration eval 啟用頻譜圖後可提供更多場景紋理資訊。

### 設計目標 vs 現狀

| 面向 | 設計目標 | 現狀 | 改進路徑 |
|------|---------|------|---------|
| L4/L5 場景辨識 | 「菜市場跟攤販對話」 | 「noisy public space」 | 頻譜圖多模態 + GEPA |
| L6 降噪強度 | 強降噪 (NR>0.5) | NR=0.3 | GEPA prompt 優化 |
| L7 回饋迴路 | 「太悶了」→ 降低 NR | semantic eval 已支援 | integration eval 測完整 harness |
